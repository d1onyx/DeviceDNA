...
/**
 * iOS SHA-256 as a self-contained pure-Kotlin implementation (avoids a CommonCrypto cinterop
 * dependency). The sync-dedup logic only requires a deterministic, stable hash per device, and
 * this is a standard SHA-256 so it also matches the Android MessageDigest output.
 */
actual fun sha256Hex(input: String): String {
    ...
    val padded = ArrayList<Byte>(bytes.size + 9)
    padded.addAll(bytes.toList())
    padded.add(0x80.toByte())
    while (padded.size % 64 != 56) padded.add(0)
    for (i in 7 downTo 0) padded.add(((bitLen ushr (i * 8)) and 0xFF).toByte())
