    ...
    LazyColumn(
        ...
        item {
            AccentCard(accentColor = colors.deviceColor) {
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(16.dp)) {
                    ...
                    if (isIos) {
                        DeviceSpec("Identifier", info.hardware.ifBlank { "Unknown" })
                    } else {
                ...
                if (info.isRooted) {
                    Spacer(Modifier.height(10.dp))
                    StatusPill(MetricStatus.Warning, if (isIos) "Jailbreak detected" else "Root detected")
                }
            }
        }
        ...
        item {
            SectionCard {
                ...
                InfoRow("Brand", info.brand, showDivider = isIos)
                if (isIos) {
                    InfoRow("Identifier", info.hardware.ifBlank { "Unknown" }, showDivider = false)
                } else {
        ...
        item {
            AccentCard(accentColor = colors.deviceColor) {
                ...
                if (isIos) {
                    InfoRow("Chip", info.socName.ifBlank { "Unknown" }, copyable = false)
                    InfoRow("Identifier", info.hardware, copyable = false)
                    InfoRow("Architecture", info.supportedAbis.joinToString(", "), copyable = false, showDivider = false)
                } else {
        ...
        item {
            SectionCard {
                ...
                InfoRow(
                    label = if (isIos) "Identifier for Vendor" else "Android ID",
        ...
        item {
            AccentCard(accentColor = if (info.isRooted) colors.warning else colors.success) {
                ...
                InfoRow(
                    label = if (isIos) "Jailbreak Detected" else "Root Detected",
                    value = if (info.isRooted) "Yes — security risk" else "No",
                    copyable = false,
                )
                InfoRow(
                    label = if (isIos) "Simulator" else "Emulator",
