...
- Subscription information when you buy or restore Premium, such as product ID,
  Google Play purchase token, subscription status, expiry time, and related
...
- Premium subscription status and Google Play verification data for real
  purchases.

The backend is built with Cloudflare Workers and Neon Postgres. Data is used to
...
- Neon Postgres for backend database storage.
...
- Дані підписки під час купівлі або відновлення Premium, зокрема product ID,
  Google Play purchase token, статус підписки, час завершення та пов'язані
  transaction/order identifiers.
...
- Статус Premium subscription і дані Google Play verification для реальних
  покупок.

Backend побудований на Cloudflare Workers і Neon Postgres. Дані
...
- Neon Postgres для backend database storage.
...
- Данные подписки при покупке или восстановлении Premium, включая product ID,
  Google Play purchase token, статус подписки, время окончания и связанные
  transaction/order identifiers.
...
- Статус Premium subscription и данные Google Play verification для реальных
  покупок.

Backend построен на Cloudflare Workers и Neon Postgres. Данные используются для
...
- Abo-Daten beim Kauf oder Wiederherstellen von Premium, wie product ID, Google
  Play purchase token, Abostatus, Ablaufzeit und zugehörige transaction/order
  identifiers.
...
- Premium subscription status und Google-Play-Verifizierungsdaten für echte Käufe.

Das Backend basiert auf Cloudflare Workers und Neon Postgres. Die Daten werden
...
- Neon Postgres für backend database storage.
