...
if grep -q 'YOUR_FIREBASE_PROJECT_ID' "$WT"; then
    echo "Set FIREBASE_PROJECT_ID in $WT before deploying." >&2
    exit 1
fi
...
# 3) Optional production secrets. Pass them as env vars when needed.
